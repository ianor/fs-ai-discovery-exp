...
async function getPersonPortraits(accessToken: string, personId: string) {
  try {
...
async function getPersonAncestry(accessToken: string, personId: string, generations: number = 4) {
  try {
...
function formatSearchResults(results: any) {
  ...
  return results.entries
    .map((entry: any) => {
      ...
      const birth = facts.find((f: any) => f.type === "http://gedcomx.org/Birth");
      const death = facts.find((f: any) => f.type === "http://gedcomx.org/Death");
...
function formatPortraitResults(results: any) {
  if (!results.portraits || results.portraits.length === 0) {
    return "No portraits found for this person.";
  }

  return results.portraits.map((portrait: any, index: number) => {
...
function formatAncestryResults(results: any) {
  if (!results.persons || results.persons.length === 0) {
    return "No ancestry information found.";
  }

  const relationships = results.relationships || [];
  const personMap = new Map(results.persons.map((p: any) => [p.id, p]));
  
  // Find parent-child relationships
  const childToParents = new Map();
  relationships
    .filter((rel: any) => rel.type === "http://gedcomx.org/ParentChild")
    .forEach((rel: any) => {
...
ID: ${person.id}`;
  ...
  function buildAncestryTree(personId: string, level: number = 0): string {
